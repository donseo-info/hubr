...
//		if( empty( $chimp_subject ) ) $chimp_subject = '';
        ...
        echo '	<tr>';
        echo '		<th><label for="for_newbie">' . __( 'Only for new users', 'wpcommunity' ) . '</label></th>';
        echo '		<td>';
        echo '			<label><input type="checkbox" id="for_newbie" name="for_newbie" value="checked" ' . checked( $for_newbie, 'checked', false ) . '> ' . __( 'Invite can activate only new users', 'wpcommunity' ) . '</label>';
        echo '		</td>';
        echo '	</tr>';
        ...
        if ( ! empty( $invite_history ) ) {
            ...
            foreach ( $invite_history as $invite_date => $invite_user_id ) {
                if ( $invite_user = get_user_by( 'ID', $invite_user_id ) ) {
                    echo '<tr>';
                    ...
                    echo '<td></td>';
                    echo '<td>';
                    echo __( '(user removed)', 'wpcommunity' );
                    echo '</td>';
                    echo '<td>';
                    echo date( 'd.m.Y H:i', $invite_date );
                    echo '</td>';
...
//		echo '	</tr>';
    ...
    public function field_container( $label = '', $name = '', $html = '' ) {
        echo '	<tr>';
        echo '		<th><label for="' . $name . '" class="chimp_subject_label">' . $label . '</label></th>';
        echo '		<td>';
        echo $html;
        echo '		</td>';
        echo '	</tr>';
...
//		$chimp_new_subject = isset( $_POST[ 'chimp_subject' ] ) ? sanitize_text_field( $_POST[ 'chimp_subject' ] ) : '';
        ...
        update_post_meta( $post_id, 'for_newbie', $for_newbie );
