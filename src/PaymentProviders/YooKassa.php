...
class YooKassa implements PaymentProviderInterface {

    const POST_META_PAYMENT_ID          = '_yookassa_payment_id';
    const POST_META_PAYMENT_METHOD_ID   = '_yookassa_payment_method_id';
    const POST_META_PAYMENT_METHOD_DATA = '_yookassa_payment_method_data';
    const POST_META_REFUND_ID           = '_yookassa_refund_id';
     ...
     * @param Logger $logger
     */
    public function __construct( $shop_id, $api_key, Logger $logger ) {
     ...
     * @see https://github.com/yoomoney/yookassa-sdk-php/blob/master/docs/examples/02-payments.md
     */
    public function create_payment( $order_id ) {
        ...
        try {
            $params = [
                ...
                'confirmation'         => [
                    'type'       => 'redirect',
                    'return_url' => $order->order_link,
                ],
                'capture'              => true,
                'description'          => '#' . $order->order_id . ' ' . $order->title,
                ...
                'merchant_customer_id' => $order->user_email,
            ];

            if ( $this->is_recurring_enabled() ) {
                // https://yookassa.ru/developers/payment-acceptance/scenario-extensions/recurring-payments#save
                $params['save_payment_method'] = true;
            }

            if ( get_setting( 'payment.yookassa.send_receipt' ) ) {
                $params['receipt'] = [
                    'customer' => [
                    ...
                    'items'    => [
                        [
                            ...
                            'measure'         => 'piece',
                            'amount'          => [
                                'value'    => $order->subscription_price,
                                'currency' => $order->currency,
                            ],
             ...
             * @since 1.0
             */
            $params = apply_filters( 'wpcommunity/yookassa/create_payment_params', $params, $order );
            ...
            $this->logger->log( "YooKassa: " . print_r( $params, true ) );

            // если все ок, сохраняем данные
            if ( $payment->getId() ) {
                add_post_meta( $order_id, self::POST_META_PAYMENT_ID, $payment->getId() );
                add_post_meta( $order_id, Orders::POST_META_PAYMENT_REFUNDABLE, $payment->getRefundable() );
            }

            $confirmation_url = $payment->getConfirmation()->getConfirmationUrl();
            if ( ! empty( $confirmation_url ) ) {
                $this->redirect_url = $confirmation_url;

                return $this;
            }

            $this->error = new WP_Error( 'cant_get_redirect_url', 'Cant get redirect url' );
        } catch ( ApiException $e ) {
            $this->error = new WP_Error( 'api_exception', "[{$e->getCode()}] " . $e->getMessage() );
        } catch ( \Exception $e ) {
            $this->error = new WP_Error( 'exception', "[{$e->getCode()}] " . $e->getMessage() );
        }

        if ( $this->error ) {
            $this->logger->log( "Error: " . $this->error->get_error_message() );
        }
    ...
    public function create_recurring_payment( $order_id, $parent_order_id ) {
        ...
        $params = [
            ...
            'capture'           => true,
            'payment_method_id' => get_post_meta( $parent_order_id, self::POST_META_PAYMENT_METHOD_ID, true ),
            'description'       => '#' . $order->order_id . ' ' . $order->title,
        ];
        ...
        try {
            ...
            $this->logger->log( "status: " . $payment->getStatus() );
            $this->logger->log( "payment id: " . $payment->getId() );
            $this->logger->log( "payment method id: " . $payment->getPaymentMethod()->getId() );

//            $payment->getPaymentMethod()->getId()
            if ( 'canceled' == $payment->getStatus() ) {

                $this->error = new WP_Error(
                    RecurringPayments::ERROR_CANCELED_ON_PAYMENT,
                    __( 'Unable to create recurring payment. Subscription was canceled by customer.', 'wpcommunity' )
                );
            } else if ( 'succeeded' == $payment->getStatus() ) {
                add_post_meta( $order_id, self::POST_META_PAYMENT_ID, $payment->getId() );
                add_post_meta( $order_id, self::POST_META_PAYMENT_METHOD_ID, $payment->getPaymentMethod()->getId() );
                add_post_meta( $order_id, Orders::POST_META_PAYMENT_REFUNDABLE, $payment->getRefundable() );
                add_post_meta( $order_id, self::POST_META_PAYMENT_METHOD_DATA, $payment->getPaymentMethod()->toArray() );
            } else {
                $this->error = new WP_Error( 'error', __( 'The recurring payment has not been created.', 'wpcommunity' ) );
            }
    ...
    protected function webhook() {
        if (
            ...
            isset( $_REQUEST['webhook_payment'] ) &&
            $_REQUEST['webhook_payment'] == 'yookassa'
        ) {

            $this->logger->log( 'webhook start' );

            $body            = @file_get_contents( 'php://input' );
            $callback_params = json_decode( $body, true );

            $this->logger->log( 'webhook body: ' . $body );

            if ( ! json_last_error() ) {
                try {
                    $this->process_request( $callback_params );
                } catch ( \Exception $e ) {
                    $this->logger->log( 'Error while process notification: ' . $e->getMessage() );
                }
            } else {
                $this->logger->log( 'Notification json error' );
    ...
    protected function process_request( $data ) {
        ...
        $notification_object = $factory->factory( $data );
        $response_object     = $notification_object->getObject();
        $client              = new Client();

        try {

            $this->logger->log( 'start complete payment' );
            $this->logger->log( 'data: ' . print_r( $data, true ) );

            if ( ! $client->isNotificationIPTrusted( $_SERVER['REMOTE_ADDR'] ) ) {
                $this->logger->log( 'fail isNotificationIPTrusted' );
                header( 'HTTP/1.1 400 Something went wrong' );
                exit();
            }

            /** Успешно оплачен и подтвержден магазином */
            if ( $notification_object->getEvent() === NotificationEventType::PAYMENT_SUCCEEDED ) {
                $someData = [
                    'paymentId'     => $response_object->getId(),
                    'paymentStatus' => $response_object->getStatus(),
                ];

                // ищем заказ по payment id
                $order_id = $this->get_order_id_by_payment_id( $response_object->getId() );
                if ( ! $order_id ) {

                    $this->logger->log( 'order not found: ' . $response_object->getId() );
                ...
                add_post_meta( $order_id, self::POST_META_PAYMENT_ID, $response_object->getId() );
                add_post_meta( $order_id, self::POST_META_PAYMENT_METHOD_ID, $response_object->getPaymentMethod()->getId() );
                add_post_meta( $order_id, self::POST_META_PAYMENT_METHOD_DATA, $response_object->getPaymentMethod()->toArray() );
                add_post_meta( $order_id, Orders::POST_META_PAYMENT_REFUNDABLE, $response_object->getRefundable() );
                ...
                if ( is_wp_error( $finish_result ) ) {
                    ...
                    // записываем сколько пришло за вычетом комиссии
                    $income = $response_object->getIncomeAmount();
                    if ( $income ) {
                        update_post_meta( $order_id, Orders::POST_META_INCOME, $income->value );
                        update_post_meta( $order_id, Orders::POST_META_INCOME_CURRENCY, $income->currency );
                    }
                    // записываем способ оплаты
                    $payment_method = $response_object->getPaymentMethod();
                    if ( $payment_method ) {
                        update_post_meta( $order_id, Orders::POST_META_PAYMENT_TYPE, $payment_method->getType() );
                    }

                    $this->logger->log( 'finish order id: ' . $order_id );
                }

//				if ($payment_method = $response_object->getPaymentMethod()) {
//					$updateData['payment_method_id'] = $payment_method->getId();
//					$payment_method->getSaved();
//				}

                exit;

                /** Успешно оплачен покупателем, ожидает подтверждения магазином */
            } elseif ( $notification_object->getEvent() === NotificationEventType::PAYMENT_WAITING_FOR_CAPTURE ) {
                $someData = [
                    'paymentId'     => $response_object->getId(),
                    'paymentStatus' => $response_object->getStatus(),
                ];
            ...
            } elseif ( $notification_object->getEvent() === NotificationEventType::PAYMENT_CANCELED ) {
                $someData = [
                    'paymentId'     => $response_object->getId(),
                    'paymentStatus' => $response_object->getStatus(),
                ];
                ...
                /** Успешный возврат */
            } elseif ( $notification_object->getEvent() === NotificationEventType::REFUND_SUCCEEDED ) {
                $someData = [
                    'refundId'     => $response_object->getId(),
                    'refundStatus' => $response_object->getStatus(),
                    'paymentId'    => $response_object->getPaymentId(),
                ];
                // ...
                // Специфичная логика
            } else {
                $this->logger->log( 'cant resolve NotificationEventType' );
                header( 'HTTP/1.1 400 Something went wrong' );
                exit();
            }
        } catch ( \Exception $e ) {
     ...
     * @return int|null
     */
    public function get_order_id_by_payment_id( $payment_id ) {
        global $wpdb;

        $meta_key = self::POST_META_PAYMENT_ID;

        $query  = "
            SELECT *
            FROM {$wpdb->prefix}postmeta
            WHERE meta_value = %s AND meta_key = '$meta_key'
        ";
        $sql    = $wpdb->prepare( $query, $payment_id );
     ...
     *
     * @return $this
     * @see https://yookassa.ru/developers/payment-acceptance/after-the-payment/refunds
     */
    public function refund( $order_id, $amount = null ) {
        ...
        $payment_id = get_post_meta( $order_id, self::POST_META_PAYMENT_ID, true );
        if ( ! $payment_id ) {
            $this->error = new WP_Error( 'payment_id_not_found', __( 'Unable to create refund without payment id', 'wpcommunity' ) );
        ...
        try {
            $refund = $client->createRefund( [
                ...
                'amount'     => [
                    'value'    => null === $amount ? $order->price : $amount,
                    'currency' => $order->currency,
                ],
            ] );

            if ( $refund->getStatus() === 'canceled' ) {
                $error  = new WP_Error( 'create_refund', __( 'Unable to create refund', 'wpcommunity' ) );
                $reason = wp_parse_args( $refund['cancellation_details'], [
                    "party"  => "",
                    "reason" => "",
                ] );
                $error->add( 'reason', json_encode( $reason ) );
//                $error->add( 'link', 'https://yookassa.ru/developers/payment-acceptance/after-the-payment/refunds#declined-refunds-cancellation-details-party' );
            ...
            if ( $refund->getStatus() === 'succeeded' ) {
                add_post_meta( $order_id, self::POST_META_REFUND_ID, $refund->getId() );
            }
...
//            }
    ...
    public function is_recurring_enabled() {
        return get_setting( 'payment.yookassa.recurring' );
    }
}
